...
def create_datafile(outfile='sample-SVO-distribution.txt'):
    """create file for data (or empty it )"""
...
if __name__ == '__main__':
    ...
    brainstructures = [item['node']['name'].lower()
                       for item in db.get_brainstructures()]
    logger.info(f"retrieved {len(brainstructures)} structures")
    brainfunctions = [item['node']['name'].lower()
                      for item in db.get_functions()]
    ...
    include_MESH = ['Rats', 'Rodent', 'Mice', 'Muridae']
    exclude_MESH = ['Humans']
    ...
    try:
        for index_status, query in enumerate(queries_to_search):
            ...
            query_pmids = q.search_pubmed_ids(
                query, searchlim=MAX_SEARCH_LIMIT)

            stats_all['queries'][f'{query}'] = query_pmids
            query_stats.add(
                **{f'found {len(query_pmids)} articles for query': f'{query}'})
            ...
            while True:
                try:
                    ...
                    if 'AB' not in record.keys():
                        filtered_pmids['excluded_by_no_abstract'].append(
                            record['PMID'])
                        if not record['PMID'] in in_database_pmids and not (DATABASE_USAGE == 'IGNORE'):
                            db.insert_article(
                                record['PMID'], record, ['ABSTRACTLESS'])
                        continue

                    # exclude irrelevant abstracts
                    if not is_relevant_abstract(record['AB'], relevant_terms):
                        record['isRelevant'] = '0'
                        filtered_pmids['excluded_by_irrelevant_abstract'].append(
                            record['PMID'])
                        if not record['PMID'] in in_database_pmids and not (DATABASE_USAGE == 'IGNORE'):
                            db.insert_article(record['PMID'], record, [
                                              'IRRELEVANT_ABSTRACT'])
                        ...
                        output_all_stats(
                            stats_all, 'intermediate-all-stats.txt')

                    # top 5 and lowest 5
                    topsizes = 5
                    pick_this_record = ((len(leftover_pmids) - rec_index) < topsizes or
                                        (len(leftover_pmids) +
                                         rec_index) < (len(leftover_pmids) + topsizes)
                                        )
                    ...
                    if pick_this_record:
                        if record['PMID'] == last_svo_id:
                            ...
                            record['SVOs'] = [
                                'abstract not relevant, no abstract']
                    ...
                    logger.exception(
                        f'error for query "{query}"" at rec_index "{rec_index}"', e)

            query_stats.add(
                **{f"articles already in dtabase for f{query}": len(in_database_pmids)})
            query_stats.add(**{reason: len(value)
                               for reason, value in filtered_pmids.items()})
            query_stats.add(
                **{f"MeSH term top 15 for {query}": meshterm_occurences.most_common(15)})
            query_stats.add(
                **{f"total # of MeSH terms for {query}": len(meshterm_occurences)})
